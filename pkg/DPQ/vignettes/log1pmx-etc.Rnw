...
%\VignetteIndexEntry{log1pmx, bd0, stirlerr - Probability Approximations for \R}
%\VignetteDepends{DPQ}
...
\title{log1pmx(), bd0(), stirlerr() -- Utilities for Poisson, Binomial, Gamma Probabilities in R}
...
$\Gamma()$ probalities \code{pgamma()} and hence indirectly e.g., for
cumulative central and non-central chisquare probablities (\code{pchisq()}).
...
\begin{align}
  \label{eq:p-D}
  p(x; n, p) = p(x; n, \frac{x}{n})  e^{-D(x;n,p)},
\end{align}
...
that the current \code{bd0()} implentation is still inaccurate in some
...
\code{c\_dt(nu)} and even more promissingly, \code{lb\_chi(nu)}.
