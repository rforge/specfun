...
%
\usepackage{parskip}
\usepackage{subfig-M}%-- subfigures 'M': less space than standard
%%
...
\Title{Computing the Beta Function for Large Arguments}
\Author{Martin B. Mächler}
...
\citeyear{MajKB73-AS64}, \cite{CraGMT77,BerKMC90}, \cite[ch.~25]{JohNKB95}),
...
which uses the gamma function can suffer severely from ``extinction'' when
...
For this and many more properties, see, e.g., \citeasnoun[ch.~6]{AbrMS70}.
...
a_nn + n \tilde{a}_{n-1}$ from which we conclude that $\tilde a_n = n!$,
since $a_nn=0$ and $\tilde{a}_0 = a_{0,1} = 1$ by definition
(\ref{def.fn}).
Hence,
...
\bibliography{Assbib,master,Ibeta-dist-mm}
