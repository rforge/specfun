...
%%__VignetteDepends{fAsianOptions}
...
\item[fAsianOptions] \emph{currently \textbf{archived} on CRAN, i.e., no
    longer easily available}.\\ It % which
  also loads \pkg{fBasics} focuses on the $I()$ and
